...
| | 标签展示 | `书名·[时代]·作者`（来自 `books_meta.json`）；**时代按原书撰写朝代**，现代译注缀作者后（如 `蒙古秘史·[元]·佚名·余大钧译注`） |
...
|----------|----------|-----------|----------|
| **先秦** | 73 部 | 史记、资治通鉴、春秋左传、论语、孟子、韩非子、庄子、楚辞、尚书、绎史、路史…… | ~43 MB |
| **秦汉** | 0（占位） | — | — |
| **魏晋南北朝** | 0（占位） | — | — |
| **隋唐五代** | 0（占位） | — | — |
...
| **明** | 0（占位） | — | — |
| **清** | 0（占位） | — | — |
| **隔离区** | `_raw_no_punct/*.txt`（35 文件已入库） | 蒙元原料、大金国志、东都事略、宋史纪事本末、七国考、路史、绎史…… | ~34 MB |
| **总计（活跃）** | **97 部** | 简体+有标点 | **~108 MB** |

> 完整书单见 [`data/books_meta.json`](data/books_meta.json)（约 122 条 meta，含尚未入活跃目录的书名）。  
...
├── README.md
├── data/
│   ├── books_meta.json     # 书目 era / author（~122 条）
│   ├── 先秦/ 秦汉/ 魏晋南北朝/ 隋唐五代/
│   ├── 辽宋夏金/ 蒙元/ 明/ 清/   # 活跃语料（简体+有标点）
│   │   └── *.txt
...
### 2026-07-18 时期筛选重命名 + 蒙元入库 + 标签规则

- **时期筛选**：`辽宋金夏`→`辽宋夏金`，`元`→`蒙元`，**去掉民国**；数据目录同步改名
- **蒙元活跃 10 部**：元史、新元史、蒙古秘史、元朝秘史、圣武亲征录、蒙鞑备录、黑鞑事略、马可波罗行纪、蒙古源流、长春真人西游记
- **书名标签规则**：时代按**原书撰写朝代**，不是现代译注出版年；现代译注缀作者后  
  - 例：`蒙古秘史·[元]·佚名·余大钧译注`（不再写 `[现代]·余大钧译注`）
- `_raw_no_punct` 补齐蒙元/大金等原料与中间本；README 统计刷新为活跃 97 部
...
- **文本语料**：来源混合（公版、维基文库、用户自行整理本）。转载或公开前请自行确认各书版权与整理本授权。
...
- 新增书 → 放 `data/<时期>/` → 更新 `books_meta.json` → **重启 8899** → 搜索抽查
- `books_meta` 的 era = **原书撰写时代**；译注本 author 可写 `佚名·某某译注`
